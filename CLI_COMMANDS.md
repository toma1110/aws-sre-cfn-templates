...
**注意: CloudFormation テンプレートを実行すると、エラー率デモアプリが自動的にセットアップされます。手動セットアップは不要です。**
...
# エラー率デモアプリのステータス確認（EC2 に SSH 接続後）
...
CloudFormation を使う場合、アプリ本体、systemd サービス、CloudWatch Agent 設定は `01-base-infrastructure.yaml` の UserData で自動作成されます。
確認だけ行う場合は以下を使います。
