...
> **注意**: 動画内では `sre-handson-base.yml` という名前で案内していますが、このリポジトリでは `01-base-infrastructure.yaml` に統一しています。互換性のため `sre-handson-base.yml` はシンボリックリンクとして残してあります（どちらを使っても同じです）。
